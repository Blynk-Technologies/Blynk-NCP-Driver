...
RpcStatus rpc_ncp_ping() {
  ...
  MessageBuffer_writeUInt16(&_rpc_buff, RPC_UID_NCP_PING);
...
bool rpc_ncp_reboot() {
  ...
  MessageBuffer_init(&_rpc_buff, rpc_output_buff, sizeof(rpc_output_buff));
  MessageBuffer_writeUInt16(&_rpc_buff, RPC_OP_INVOKE);
  MessageBuffer_writeUInt16(&_rpc_buff, RPC_UID_NCP_REBOOT);
  MessageBuffer_writeUInt16(&_rpc_buff, ++_rpc_seq);
...
bool rpc_ncp_hasUID(uint16_t uid) {
  ...
  MessageBuffer_init(&_rpc_buff, rpc_output_buff, sizeof(rpc_output_buff));
  MessageBuffer_writeUInt16(&_rpc_buff, RPC_OP_INVOKE);
  MessageBuffer_writeUInt16(&_rpc_buff, RPC_UID_NCP_HASUID);
  MessageBuffer_writeUInt16(&_rpc_buff, ++_rpc_seq);
